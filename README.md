...
PyPI の標準 `ctranslate2` は ROCm 非対応のため、ROCm 対応の wheel を別途入手して `--with` で追加する必要があります。

```bash
# ROCm 対応 ctranslate2 の wheel を入手後（例: ローカルファイル）
uv run --with ./ctranslate2_rocm-*.whl transcribe.py meeting_folder/ --device cuda
```

ROCm 対応ビルドの入手先は [OpenNMT/CTranslate2](https://github.com/OpenNMT/CTranslate2) を参照してください。
